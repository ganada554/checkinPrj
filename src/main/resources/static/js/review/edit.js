window.addEventListener("load", function() {
    ...
    let imgCancelBtn = document.querySelector(".img-cancel-btn");
	...
	let prevHselected = hashtag.querySelector(".selected");
	let prevRselected = rating.querySelector(".selected"); 
	let hselected = null;
	let rselected = null;
    ...
    hashtag.onclick = function(e) {
    	...
    	if(hselected != null && hselected != e.target) {
            ...
            hselected.nextElementSibling.removeAttribute('name', "hashtagId");
		}
		
	    // 불러온 hashtag값이 아닌 다른 값 선택 시 불러온 값에 있던 스타일, input name 속성 제거
		prevHselected.classList.remove("selected");
		prevHselected.nextElementSibling.removeAttribute('name', "hashtagId");
		...
		hselected.nextElementSibling.setAttribute('name', "hashtagId");
	}
	...
	rating.onclick = function(e) {
    	...
    	if(rselected != null && rselected != e.target) {
			...
			rselected.nextElementSibling.removeAttribute('name', "ratingId");	
		}
		
		// 불러온 rating 값이 아닌 다른 값 선택 시 불러온 값에 있던 스타일, input name 속성 제거
		prevRselected.classList.remove("selected");
		prevRselected.nextElementSibling.removeAttribute('name', "ratingId");
        
        rselected = e.target;
        rselected.classList.toggle("selected");
        
        // selected된 input의 name값 지정	
		rselected.nextElementSibling.setAttribute('name', "ratingId");
	}
	...
	submitBtn.onclick = function(e) {
		...
		request.open("POST", "/review/edit", true);
